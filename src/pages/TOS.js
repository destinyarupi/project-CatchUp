...
import { TACLinks, TAC } from '../data/Data'

export const TOS = () => {
  const links = TACLinks.map(data => (
    <a href={data.link} key={data.link} className='mb-3 text-decoration-none text-black d-block'>
      <small>{data.title}</small>
    </a>
  ))
  const TOSData = TAC.map(data => (
          ...
          <Row>
            ...
            <Col md={9}>
              <div className='mb-4'>
                <h2>Terms of Use</h2>
                <small>
                  This document was last updated 15th of December,
                  2022. To start using our product, read this Terms
